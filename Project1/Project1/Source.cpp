...
int main(){
	...
	Property* prop2 = new Property("pras", GlobalConstants::PropertyColors::BLUE, 1500, 100, rent, 1);

	std::cout << prop->getFieldPrice() << std::endl;
	std::cout << prop->getPriceForBuilding() << std::endl;
	std::cout << prop->getOwnerId() << std::endl;
	std::cout << prop->getRentPrice() << std::endl;

	Player* player = new Player("Pedal");
